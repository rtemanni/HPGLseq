...
#######################################
##### DE analysis core function called  by DEAnalysis
#######################################

DERes= function(cds,cond1,cond2,Design.file,anot.file,adjpval,source.format,SelFeature,fields,cleandescription)
{
  nprefix=basename(Design.file)
  print("Running Stat Test")
  res <- nbinomTest( cds, as.character(cond1), as.character(cond2))
  print("Generatting MA plot")
  png(paste(name.prefix,"_",as.character(cond1),"_", as.character(cond2),"_res_ma_plot.png",sep=""))
  plotMA(res)
  dev.off()
  # clean 
  res[sapply(res, is.infinite)] = NA
  names(res)[match("baseMeanA", names(res))] = as.character(cond1)
  names(res)[match("baseMeanB", names(res))] = as.character(cond2)
  
  print("Anotating data")
  res=res[order(res[ ,8]), ]
  resAnot=annotate.data(res,anot.file,source.format,SelFeature,fields,cleandescription)
  write.table(resAnot,paste(name.prefix,"_",as.character(cond1),"_", as.character(cond2),"_res_anot.xls",sep=""),sep="\t",row.names = F)
  
  resSig=getsig.deseq(res)
  print(dim(resSig))
  resSigAanot=annotate.data(resSig,anot.file,source.format,SelFeature,fields,cleandescription)
  write.table(resSigAanot,paste(name.prefix,"_",as.character(cond1),"_", as.character(cond2),"_res_sig_anot.xls",sep=""),sep="\t",row.names = F)
  print("generate Bed Track")
  sv2bed(resSigAanot,cond1,cond2,nprefix)
...
#######################################
##### select DE genes based on AdjP value
#######################################
getsig.deseq=function(res,adjpval=.05)
{
  tmp <- res[ res$padj < adjpval, ]
  tmpc=tmp[which(!is.na(tmp[,8])),]
  #tmpc=tmpc[order(abs(tmpc[ ,6]),decreasing=T), ]
  tmpc=tmpc[order(tmpc[ ,8]), ]
  tmpc=cbind(tmpc,rank=(1:dim(tmpc)[1]))
  return(tmpc)
  #tmp[1:20,]
}
...
#######################################
##### clean up description by converting ascii code to char
#######################################
...
#######################################
##### auto install packages from Cran and Bioconductor and dependecies if they are not installed
#######################################
